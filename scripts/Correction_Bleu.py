...
if __name__ == "__main__":
    ...
    for img_path in image_paths:
        ...
        # Lecture image
        mat_bgr = cv2.imread(str(img_path))
        if mat_bgr is None:
            print(f"  Erreur lecture image : {img_path}")
            continue

        # Conversion pour la fonction correct()
        mat_rgb = cv2.cvtColor(mat_bgr, cv2.COLOR_BGR2RGB)

        try:
            corrected_bgr = correct(mat_rgb)   # ta fonction retourne déjà du BGR
        except Exception as e:
            print(f"  Erreur pendant la correction : {e}")
            continue

        # Image comparative : originale à gauche, corrigée à droite
        comparison = np.hstack((mat_bgr, corrected_bgr))

        # Nom du fichier de sortie
        out_path = OUTPUT_DIR / img_path.name

        # Sauvegarde
        success = cv2.imwrite(str(out_path), comparison)
